...
$status_const2 = parse_ini_file($_SERVER['DOCUMENT_ROOT'] . "/config_date_cust_leads.ini");
...
if (!$conn) {
    $content = "CUSTOMFIELDS_LEADS mysql Connection failed: ".date('h:i:s') . ' Код: '. mysqli_connect_error() . "\n";
...
//write to log.txt 'CUSTOMFIELDS_LEADS Connected successfully';
    $content = "CUSTOMFIELDS_LEADS mysql Connected successfully ".date('h:i:s') ."\n";
    $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/log.txt","a");
    fwrite($fp, $content);
    fclose($fp);}
...
#Устанавливаем необходимые опции для сеанса cURL
    ...
    try {
        ...
        $content = "CUSTOMFIELDS_LEADS curl Connection failed: " . date('h:i:s') . ' error: ' . $code . "\n";
        $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/log.txt", "a");
        fwrite($fp, $content);
        fclose($fp);
        sleep(14400);
...
//                    Запись изменений в таблицу CUSTOMFIELDS_LEADS
                    ...
                    if (mysqli_query($conn, $sql1)) {
                        $content = "Запись изменений в таблицу CUSTOMFIELDS_LEADS удалась " . date('h:i:s') . "\n";
                        $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/log.txt", "a");
                        fwrite($fp, $content);
                        fclose($fp);
                    } else {
                        $content = "Запись изменений в таблицу CUSTOMFIELDS_LEADS не удалась " . date('h:i:s') . ' Код: ' . mysqli_error($conn) . "\n";
                        $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/log.txt", "a");
                        fwrite($fp, $content);
                        fclose($fp);
                    }
...
////            sleep(0.1);
                    ...
                    if (mysqli_query($conn, $sql2)) {
                        $content = "Запись в таблицу CUSTOMFIELDS_LEADS удалась " . date('h:i:s') . "\n";
                        $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/log.txt", "a");
                        fwrite($fp, $content);
                        fclose($fp);
                    } else {
                        $content = "Запись в таблицу CUSTOMFIELDS_LEADS не удалась " . date('h:i:s') . ' Код: ' . mysqli_error($conn) . "\n";
                        $fp = fopen($_SERVER['DOCUMENT_ROOT'] . "/log.txt", "a");
                        fwrite($fp, $content);
                        fclose($fp);
                    }
    ...
    $dd = date(DATE_RFC2822, $dt);
    file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/config_date_cust_leads.ini", "date = $dm");
}
mysqli_close($conn);
